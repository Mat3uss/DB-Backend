...
alter table tbl_atores add foreign key(tbl_ator_filme_id) 
REFERENCES tbl_ator_filme (id);

alter table tbl_atores add foreign key(tbl_diretor_filme_id) 
...
references tbl_diretores(id_diretor)
);
...
id_filme int not null,
foreign key(id_genero) references tbl_genero_filme(id)
);
...
insert into tbl_diretores (nome,data_nascimento, foto, biografia, nacionalidadediretor_id, sexo_id) values(
'Quentin Tarantino',
'1963-03-27',
